...
while IFS= read -r sub_url; do
    if [ -z "$sub_url" ]; then # 确保 sub_url 非空
        continue
    fi
    # curl -s 已静默输出，grep 不会产生过多输出到 stdout
...
# 不再直接 cat "$TEMP_SIMPLE_TEST_PASS" 到 stdout，减少日志
...
# Python 脚本内部不再打印每个转换的节点信息到 stdout
python3 -c '
import sys
import base64
import json
...
def parse_url(url):
    try:
        if url.startswith("vmess://"):
            ...
            # 不打印未知协议，只记录到 stderr (不会出现在GitHub Actions主日志中，除非有错误)
...
with open(sys.argv[2], "a") as config_file:
...
# 使用 Mihomo 的外部控制器 API 进行测试，现在 Python 脚本内部将不会打印每个节点的测试结果
# 而是会汇总信息
python3 -c '
import requests
import json
...
RESULTS_FILE = sys.argv[1] # 指向 data/all.txt 的路径
